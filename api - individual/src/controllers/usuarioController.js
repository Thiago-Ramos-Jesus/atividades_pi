...
function cadastrar(req, res) {
    ...
    var email = req.body.emailServer;
    var senha = req.body.senhaServer;
    var cargo = req.body.permissaoServer;
    ...
    if (nome == undefined) {
        ...
        res.status(400).send("Permisão está undefined!");
    } else {
...
function dashboard(req, res) {
    ...
    if (unidade == undefined) {
        ...
        usuarioModel.dashboard(unidade)
            .then(
                function (resultado) {
                    console.log(`\nResultados encontrados: ${resultado.length}`);
                    console.log(`Resultados: ${JSON.stringify(resultado)}`); // transforma JSON em String
                     ...
                     if (resultado.length == 1) {
                        ...
                        res.status(403).send("Unidade invalida");
                    }
                }
            ).catch(
                function (erro) {
                    console.log(erro);
                    console.log("\nHouve um erro ao acessar a dashboard! Erro: ", erro.sqlMessage);
                    res.status(500).json(erro.sqlMessage);
                }
            );
    }
